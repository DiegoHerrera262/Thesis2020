...
class VariationalSpinGraph(HeisenbergGraph):

    '''
    Class for simulating a generic Heisenberg hamiltonian
    defined on a graph, with a variational approach
    '''
    ...
    def __init__(self,
        ...
        self.spinInteractions = spinInteractions
        self.externalField = externalField
        self.graph = gr.generateVariationalGraph(
            spinInteractions, externalField)
        # Color the graph for parallelisation
        if withColoring:
            self.matching, self.graphColors = gr.colorMatching(self.graph)
        else:
        ...
        try:
            self.optimizationMethod = kwargs['optimizationMethod']
        except KeyError:
            self.optimizationMethod = 'BFGS'
        # Setting up simulation environment
        try:
            self.localSimulation = kwargs['localSimulation']
            if not self.localSimulation:
